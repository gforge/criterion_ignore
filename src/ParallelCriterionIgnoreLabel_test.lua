  ...
  criterion_ignore_test['tostring' .. test_name] =
    function ()
      mytester:assert(tostring(component):find(
                      ...
                      'criterium components should have a descriptive tostring' ..
                      ' beginning with the classname')
    end
end

function criterion_ignore_test.Parrallel()
   local input = {torch.rand(2,10), torch.randn(2,10)}
   local target = {torch.IntTensor{1,8}, torch.randn(2,10)}
   local nll = nn.ClassNLLCriterion()
   local mse = nn.MSECriterion()
   local pc = criterion_ignore.Parallel():add(nll, 0.5):add(mse)
   ...
   local pc1 = criterion_ignore.Parallel(true):add(mse1, 0.5):add(mse1:clone())
   local output1_1 = pc1:forward(input1, target1)
   local output1_2 = mse1:forward(input1[1], target1)/2 + mse1:forward(input1[2], target1)
   mytester:assert(math.abs(output1_2 - output1_1) < 0.00001, "ParrallelCriterionIgnoreLabel repeatTarget forward error")
   ...
   mytester:assertTensorEq(gradInput1_1[1], gradInput1_2[1], 0.000001, "ParrallelCriterionIgnoreLabel repeatTarget backward error 1")
   mytester:assertTensorEq(gradInput1_1[2], gradInput1_2[2], 0.000001, "ParrallelCriterionIgnoreLabel repeatTarget backward error 2")
   ...
   local pc2_1 = criterion_ignore.Parallel():add(nll2_1, 0.5):add(mse2)
   local pc2_2 = criterion_ignore.Parallel():add(nll2_2, 0.4):add(pc2_1)
   ...
   mytester:assert(math.abs(output2_2 - output2_1) < 0.00001, "ParrallelCriterionIgnoreLabel table forward error")
   local gradInput2_2 = {
   ...
   mytester:assertTensorEq(gradInput2_1[1], gradInput2_2[1], 0.000001, "ParrallelCriterionIgnoreLabel table backward error 1")
   mytester:assertTensorEq(gradInput2_1[2][1], gradInput2_2[2][1], 0.000001, "ParrallelCriterionIgnoreLabel table backward error 2")
   mytester:assertTensorEq(gradInput2_1[2][2], gradInput2_2[2][2], 0.000001, "ParrallelCriterionIgnoreLabel table backward error 3")
   ...
   local pc3 = criterion_ignore.Parallel():add(nll3_1, 0.5):add(nll3_2, 1, 0)
   ...
   mytester:assert(math.abs(output3_2 - output3_1) < 0.00001, "ParrallelCriterionIgnoreLabel forward error")
   ...
   mytester:assertTensorEq(gradInput3_1[1], gradInput3_2[1], 0.000001, "ParrallelCriterionIgnoreLabel backward error 4")
   mytester:assertTensorEq(gradInput3_1[2], gradInput3_2[2], 0.000001, "ParrallelCriterionIgnoreLabel backward error 5")
   
   -- try to ignore one side
   target3[2] = 0
   local output3_3 = pc3:forward(input3, target3)
   local output3_4 = nll3_1:forward(input3[1], target3[1])/2
   mytester:assert(math.abs(output3_4 - output3_3) < 0.00001, "ParrallelCriterionIgnoreLabel ignore doesn't work out as planned")
   
   local gradInput3_4 = nll3_1:backward(input3[1], target3[1]):clone():div(2)
   local gradInput3_3 = pc3:backward(input3, target3)
   mytester:assertTensorEq(gradInput3_3[1], gradInput3_4, 0.000001, "ParrallelCriterionIgnoreLabel backward error with missing 6")
   mytester:assert(torch.sum(gradInput3_3[2]) < 0.000001, "ParrallelCriterionIgnoreLabel backward error with missing fails to ignore weights 7")
end

mytester:add(criterion_ignore_test)

if not nn_criterion_ignore then
